# ============================================
# Project: Active Directory Account Audit Toolkit
# Author: Brittany
# Description:
# Menu-driven PowerShell toolkit for common
# Active Directory account support tasks
# ============================================

# Check for Active Directory module
if (-not (Get-Module -ListAvailable -Name ActiveDirectory)) {
    Write-Host "Active Directory module is not installed or available." -ForegroundColor Red
    Write-Host "Run this script on a machine with RSAT / AD tools installed." -ForegroundColor Yellow
    exit
}
...
function Show-Menu {
    ...
    Write-Host "5. Find users that have never logged on"
    Write-Host "6. Exit"
    Write-Host ""
}
...
function Show-LockedAccounts {
    ...
    try {
        $lockedUsers = Search-ADAccount -LockedOut -UsersOnly | Select-Object Name, SamAccountName
...
function Unlock-UserAccountTool {
    ...
    try {
        ...
        Write-Host "Unable to unlock account '$username'." -ForegroundColor Red
        Write-Host "Verify the username and try again." -ForegroundColor Yellow
...
function Show-ExpiredAccounts {
    ...
    try {
        $expiredUsers = Search-ADAccount -AccountExpired -UsersOnly | Select-Object Name, SamAccountName
...
function Show-NeverLoggedOnUsers {
    Write-Host "`nRetrieving users that have never logged on..." -ForegroundColor Yellow

    try {
        $neverLoggedOnUsers = Get-ADUser -Filter * -Properties LastLogonDate |
            Where-Object { -not $_.LastLogonDate } |
            Select-Object Name, SamAccountName

        if ($neverLoggedOnUsers) {
            $neverLoggedOnUsers | Format-Table -AutoSize
        }
        else {
            Write-Host "No users were found that have never logged on." -ForegroundColor Green
        }
    }
    catch {
        Write-Host "Error retrieving users that have never logged on: $($_.Exception.Message)" -ForegroundColor Red
    }
